...
def keep_session_settings(none):
    preferences = preferences(bpy.context)
    stack = namestack.options(bpy.context)
...
class version:
    ...
    def string(bl_info):
        return '{}.{}'.format(str(bl_info['version'][0]), bl_info['version'][1])
    ...
    def remote_info():
        return requests.get('{}{}/{}/{}/update_info'.format(remote['raw'], remote['user'], remote['repo'], remote['branch'])).text
...
class namestack:
    ...
    class stack:
        ...
        def __new__(self, context):
            ...
            option = namestack.options(context).filter_options['options']
            ...
            if option.display_mode == 'ACTIVE':
                ...
                elif context.active_object and preferences(context).pin_active:
                    stack = {
                        'objects': {},
                        'datablocks': []}
            ...
            if 'datablocks' in stack:
                if context.active_object and preferences(context).pin_active and option.display_mode != 'ACTIVE':
                ...
                for object in stack['datablocks']:
                    ...
                    for type in namestack.stack.types:
                        if type != 'object_data':
                            if getattr(option, type):
                                if getattr(self, type)(context, object):
                                    ...
                                    stack['objects'][object.name][type] = {
                                        'datablocks': getattr(namestack.stack, type)(context, object)}
        ...
        def objects(context, option, location):
            return sorted([object for object in location if object != context.active_object], key=lambda object: object.name) if preferences(context).pin_active else sorted([object for object in location], key=lambda object: object.name)
        ...
        def bones(context, object):
            ...
            if object == context.active_object and object.type == 'ARMATURE':
                option = namestack.options(context).filter_options['options']
                display_mode = namestack.options(context).filter_options['options'].display_mode
                ...
                if context.mode == 'POSE':
                    if option.display_mode == 'ACTIVE':
                        ...
                        bones = [pose_bone for pose_bone in context.selected_pose_bones if pose_bone != context.active_pose_bone] if preferences(context).pin_active else [pose_bone for pose_bone in context.selected_pose_bones]

                    else:
                        bones = [pose_bone for pose_bone in context.visible_pose_bones if pose_bone != context.active_pose_bone] if preferences(context).pin_active else [pose_bone for pose_bone in context.visible_pose_bones]

                    if preferences(context).pin_active and option.display_mode != 'ACTIVE':
                    ...
                    if option.display_mode == 'ACTIVE':
                        ...
                        bones = [bone for bone in context.selected_bones if bone != context.active_bone] if preferences(context).pin_active else [bone for bone in context.selected_bones]

                    else:
                        bones = [bone for bone in context.visible_bones if bone != context.active_bone] if preferences(context).pin_active else [bone for bone in context.visible_bones]
...
class datablock:
    ...
    def options(context):
        ...
        location = preferences(context).datablock_options
    ...
    def contexts(scene, context):
        ...
        if context.active_object:
            ...
            if context.active_object.type in {'CURVE', 'LATTICE', 'MESH', 'SURFACE', 'FONT'}:
                items.append(('MODIFIER', 'Modifier', '', 'MODIFIER', 6))

            if context.active_object.type in {'CURVE', 'SURFACE', 'FONT'}:
                items.append(('DATA', 'Data', '', 'CURVE_DATA', 7))

            else:
                items.append(('DATA', 'Data', '', '{}_DATA'.format(context.active_object.type), 7))

            if context.active_object.type in {'CURVE', 'MESH', 'META', 'SURFACE', 'FONT'}:
                items.append(('MATERIAL', 'Material', '', 'MATERIAL_DATA', 8))
...
class update:
    ...
    def handlers(context, remove=False):
        if preferences(context).keep_session_settings:
            if not remove:
    ...
    def item_panel_poll(restore=False):
        if restore:
            bpy.types.VIEW3D_PT_view3d_name.poll = item_panel_poll

        else:
            bpy.types.VIEW3D_PT_view3d_name.poll = item_panel_poll_override
    ...
    def operation_name(operator, context):
        ...
        active_operation.name = batchname.operation_name(active_operation)
    ...
    def target_options(operator, context):
        option = batchname.options(context).target_options['options']

        if option.toggle_objects:
            for target in batchname.catagories['Objects']:
        ...
        if option.toggle_objects_data:
            for target in batchname.catagories['Objects Data']:
                setattr(option, target, True)
